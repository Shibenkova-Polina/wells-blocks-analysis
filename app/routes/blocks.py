...
def get_dashboard_data():
    """Полная реализация дашборда блока как в app.py"""
    try:
        ...
        # Определение block_id и block_name как в app.py
        ...
        charts_data = {
            'dist_deviations': [{'name': row[0], 'deviation': float(row[5])} for row in dist_deviations],
            'length_deviations': [{'name': row[0], 'diff': float(row[3])} for row in length_deviations],
            'diameter_deviations': [{'name': row[0], 'diff': float(row[3])} for row in diameter_deviations],
            'angle_deviations': [{'name': row[0], 'diff': float(row[3])} for row in direction_deviations],
            'azimuth_deviations': [{'name': row[0], 'diff': float(row[6])} for row in direction_deviations if row[6] is not None]
        }

        # Выявление критических отклонений как в app.py
        critical_deviations = {
            'dist': [],
            'length': [],
            'diameter': [],
        ...
        for row in length_deviations:
            if row[1] and row[2] and abs(float(row[3])) > float(row[1]) * 0.1:
        ...
        for row in direction_deviations:
            if row[4] and row[5] and row[6] is not None:
        ...
        return render_template('dashboard.html',
                            ...
                            critical_deviations=json.dumps(critical_deviations),
...
def get_block_info(block_id):
    """Получение информации о блоке как в app.py"""
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    try:
        ...
        if result:
            return {
                'crush_energy': result[0],
                'default_hole_space': result[1],
                'default_row_distance': result[2],
                'rock_name': result[3],
                'rock_rigidity': result[4],
                'rock_density': result[5]
            }
        return {}
    except Exception as e:
...
# Дополнительные маршруты для 3D визуализации как в app.py
